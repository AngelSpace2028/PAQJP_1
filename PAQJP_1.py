...
class StateTable:
    def __init__(self):
        self.table = [
            ...
            [62, 119, 2, 9], [62, 119, 2, 9], [88, 121, 1, 10], [88, 121, 1, 10], [90, 122, 0, 11], [123, 85, 12, 0],
            [124, 97, 11, 1], [124, 97, 11, 1], [126, 57, 10, 2], [126, 57, 10, 2],
            [62, 129, 2, 10], [62, 129, 2, 10], [98, 131, 1, 11], [98, 131, 1, 11],
            [90, 132, 0, 12], [133, 85, 13, 0], [134, 97, 12, 1], [134, 97, 12, 1],
            [136, 57, 11, 2], [136, 57, 11, 2], [62, 139, 2, 11], [62, 139, 2, 11],
            [98, 141, 1, 12], [98, 141, 1, 12], [90, 142, 0, 13], [143, 95, 14, 0],
            [144, 97, 13, 1], [144, 97, 13, 1], [68, 57, 12, 2], [68, 57, 12, 2],
            [62, 81, 2, 12], [62, 81, 2, 12], [98, 147, 1, 13], [98, 147, 1, 13],
            [100, 148, 0, 14], [149, 95, 15, 0], [150, 107, 14, 1], [150, 107, 14, 1],
            [108, 151, 1, 14], [108, 151, 1, 14], [100, 152, 0, 15], [153, 95, 16, 0],
            [154, 107, 15, 1], [108, 155, 1, 15], [100, 156, 0, 16], [157, 95, 17, 0],
            [158, 107, 16, 1], [108, 159, 1, 16], [100, 160, 0, 17], [161, 105, 18, 0],
            [162, 107, 17, 1], [108, 163, 1, 17], [110, 164, 0, 18], [165, 105, 19, 0],
            [166, 117, 18, 1], [118, 167, 1, 18], [110, 168, 0, 12], [169, 105, 20, 0],
            [170, 117, 19, 1], [118, 171, 1, 19], [110, 172, 0, 20], [173, 105, 21, 0],
            [174, 117, 20, 1], [118, 175, 1, 20], [110, 176, 0, 21], [177, 105, 22, 0],
            [178, 117, 21, 1], [118, 179, 1, 21], [110, 180, 0, 22], [181, 115, 23, 0],
            [182, 117, 22, 1], [118, 183, 1, 22], [120, 184, 0, 23], [185, 115, 24, 0],
            [186, 127, 23, 1], [128, 187, 1, 23], [120, 188, 0, 24], [189, 115, 25, 0],
            [190, 127, 24, 1], [128, 191, 1, 24], [120, 192, 0, 25], [193, 115, 26, 0],
            [194, 127, 25, 1], [128, 195, 1, 25], [120, 196, 0, 26], [197, 115, 27, 0],
            [198, 127, 26, 1], [128, 199, 1, 26], [120, 200, 0, 27], [201, 115, 28, 0],
            [202, 127, 27, 1], [128, 203, 1, 27], [120, 204, 0, 28], [205, 115, 29, 0],
            [206, 127, 28, 1], [128, 207, 1, 28], [120, 208, 0, 29], [209, 125, 30, 0],
            [210, 127, 29, 1], [128, 211, 1, 29], [130, 212, 0, 30], [213, 125, 31, 0],
            [214, 137, 30, 1], [138, 215, 1, 30], [130, 216, 0, 31], [217, 125, 32, 0],
            [218, 137, 31, 1], [138, 219, 1, 31], [130, 220, 0, 32], [221, 125, 33, 0],
            [222, 137, 32, 1], [138, 223, 1, 32], [130, 224, 0, 33], [225, 125, 34, 0],
            [226, 137, 33, 1], [138, 227, 1, 33], [130, 228, 0, 34], [229, 125, 35, 0],
            [230, 137, 34, 1], [138, 231, 1, 34], [130, 232, 0, 35], [233, 125, 36, 0],
            [234, 137, 35, 1], [138, 235, 1, 35], [130, 236, 0, 36], [237, 125, 37, 0],
            [238, 137, 36, 1], [138, 239, 1, 36], [130, 240, 0, 37], [241, 125, 38, 0],
            [242, 137, 37, 1], [138, 243, 1, 37], [130, 244, 0, 38], [245, 135, 39, 0],
            [246, 137, 38, 1], [138, 247, 1, 38], [140, 248, 0, 39], [249, 135, 40, 0],
            [250, 69, 39, 1], [80, 251, 1, 39], [140, 252, 0, 40], [249, 135, 41, 0],
            [250, 69, 40, 1], [80, 251, 1, 40], [140, 252, 0, 41]
        ]
...
def encode_with_compression(compressor):
    print("\nSimple Encoder (XOR transformation + Compression)")
    try:
        input_file = input("Enter input file: ").strip()
        output_base = input("Enter output base name (without .enc): ").strip()
    except EOFError:
        logging.error("No input detected. Exiting encode mode.")
        return
    ...
    try:
        ...
        compressed_data = compressor.compress_with_best_method(original_data)
        ...
        logging.info(f"Half of size: {half_size}")
        logging.info(f"Nearest prime around {half_size} is: {nearby_prime}")
        logging.info(f"Encoding complete. Output saved to {output_enc}")
    except Exception as e:
        logging.error(f"An error occurred during encoding: {e}")

def decode_with_compression(compressor):
    print("\nSimple Decoder (Decompression + XOR transformation)")
    try:
        input_enc = input("Enter encoded file (.enc): ").strip()
        output_file = input("Enter output file: ").strip()
    except EOFError:
        logging.error("No input detected. Exiting decode mode.")
        return

    if not os.path.isfile(input_enc):
        logging.error(f"Error: File '{input_enc}' does not exist.")
        return
    ...
    try:
        ...
        recovered_data = compressor.decompress_with_best_method(encoded_data)

        with open(output_file, 'wb') as f:
            f.write(recovered_data)

        logging.info(f"Decoding complete. Output saved to {output_file}")
    except Exception as e:
        logging.error(f"An error occurred during decoding: {e}")
...
def main():
    ...
    print("3 - Encode file (XOR + Compression)")
    print("4 - Decode file (Decompression + XOR)")
    print("5 - Compress file (Huffman/zlib)")
    print("6 - Decompress file (Huffman/zlib)")
    ...
    if choice in ('1', '2', '5', '6'):
        ...
        if choice == '1':
            ...
            try:
                ...
                if attempts < 1 or iterations < 1:
                    logging.error("Attempts and iterations must be 1 or greater.")
                    return
            except ValueError:
                logging.error("Invalid input. Please enter integers for attempts and iterations.")
                return

            with open(input_file, 'rb') as f:
                data_bytes = f.read()

            output_file = output_file if output_file else input_file + '.bin'

            if len(data_bytes) < HUFFMAN_THRESHOLD:
                compressed_data = compressor.compress_data_huffman(compressor.file_to_binary(input_file))
                if not compressor.binary_to_file(compressed_data, output_file):
                    logging.error(f"Error saving compressed file: {output_file}")
                    return
            else:
                compressed_data = compressor.compress_data_zlib(data_bytes)
                if compressed_data is None:
                    return
                with open(output_file, 'wb') as f:
                    f.write(compressed_data)
            ...
            output_file = output_file if output_file else input_file[:-4]  # Remove .bin extension

            try:
                decompressed_data = compressor.decompress_data_zlib(compressed_data)
                if decompressed_data:
                    with open(output_file, 'wb') as f:
                        f.write(decompressed_data)
                    logging.info("Decompressed using zlib.")
                else:
                    compressed_binary = compressor.file_to_binary(input_file)
                    if compressed_binary:
                        decompressed_data = compressor.decompress_data_huffman(compressed_binary)
                        if decompressed_data and compressor.binary_to_file(decompressed_data, output_file):
                ...
                logging.error(f"An error occurred during decompression: {e}")
                return
        ...
        encode_with_compression(compressor)
    
    elif choice == '4':
        decode_with_compression(compressor)
