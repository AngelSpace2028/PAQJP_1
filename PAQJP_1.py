...
def save_date_to_file(compressor, date_str, filename):
    """Save the date string to a file using SmartCompressor's binary_to_file method."""
    binary_str = ''.join(format(ord(c), '08b') for c in date_str)
    success = compressor.binary_to_file(binary_str, filename)
    if success:
        logging.info(f"Date saved to {filename}")
    else:
        logging.error(f"Failed to save date to {filename}")

def main():
    print("PAQJP_1 Compression System with Date Saving")
    print("Created by Jurijus Pacalovas.")
    print("Options:")
    print("1 - Compress file (PAQJP_1 with transformations)")
    print("2 - Decompress file (PAQJP_1 with transformations)")
    print("3 - Save calculated date to file")
    ...
    if choice == '3':
        output_file = input("Output file name for date (e.g., calculated_date.txt): ").strip()
        if not output_file:
            output_file = "calculated_date.txt"
        
        # Calculate the date
        start_date = datetime(2025, 7, 10, 12, 40)  # July 10, 2025, 12:40 PM IST
        total_seconds = 6_691_736_460  # 60s + 60m + 24h + 31d + 12m + 356d + 7w + 2100y
        end_date = start_date + timedelta(seconds=total_seconds)
        date_str = end_date.strftime("%Y-%m-%d %I:%M:%S %p IST")
        
        # Save to file
        save_date_to_file(compressor, date_str, output_file)
        logging.info(f"Calculated date: {date_str}")
        return
