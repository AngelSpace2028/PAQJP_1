...
# Transformation functions from PAQJP_1
...
# Transformation functions from 03
...
# Transformation functions from 00
...
def reverse_random_minus_blocks(transformed_data, metadata, block_size_bits=64):
    block_size = block_size_bits // 8
    if block_size == 0:
        raise ValueError("Block size cannot be 0 bytes")
    ...
    metadata_idx = 0
    for i in range(0, len(transformed_data), block_size):
        block = transformed_data[i:i + block_size]
        if len(block) < block_size:
            block += bytes(block_size - len(block))  # Padding
        rand_bytes = metadata[metadata_idx:metadata_idx + block_size]
        metadata_idx += block_size
        original_block = bytes([(b + rand_bytes[j % block_size]) % 256 for j, b in enumerate(block)])
...
def remove_block_size_64(data):
    transformed_data = bytearray()
    i = 0
    while i < len(data):
        if i + 2 > len(data):
            break  # Incomplete block size
        block_size = int.from_bytes(data[i:i+2], 'big')
        i += 2
        if block_size != 64:
            raise ValueError(f"Unexpected block size: {block_size}")
        block = data[i:i + block_size]
        if len(block) < block_size:
...
class SmartCompressor:
    ...
    def transform_05(self, data):
        transformed, metadata = random_minus_blocks(data, block_size_bits=64)
        return transformed + metadata  # Append metadata for decompression
    
    def reverse_transform_05(self, data):
        block_size = 64 // 8  # 8 bytes
        data_size = len(data) - (len(data) // (block_size + 1)) * block_size  # Estimate data size
        transformed_data = data[:data_size]
        metadata = data[data_size:]
        return reverse_random_minus_blocks(transformed_data, metadata, block_size_bits=64)
    
    def transform_06(self, data):
        return add_block_size_64(data)
    
    def reverse_transform_06(self, data):
        return remove_block_size_64(data)
    
    def transform_07(self, data):
        return rle_encode_1byte(data)
    
    def reverse_transform_07(self, data):
        return rle_decode_1byte(data)
    
    def transform_08(self, data):
        return reverse_chunk(data)
    
    def reverse_transform_08(self, data):
        return reverse_chunk(data)  # Self-inverse
    
    def transform_09(self, data):
        return add_random_noise(data)
    
    def reverse_transform_09(self, data):
        return data  # Non-reversible, return as-is
    
    def transform_10(self, data):
        return subtract_1_from_each_byte(data)
    
    def reverse_transform_10(self, data):
        return add_1_to_each_byte(data)
    
    def transform_11(self, data):
        return move_bits_left(data, random.randint(1, 7))
    
    def reverse_transform_11(self, data):
        return move_bits_right(data, random.randint(1, 7))  # Approximate inverse
    
    def transform_12(self, data):
        return move_bits_right(data, random.randint(1, 7))
    
    def reverse_transform_12(self, data):
        return move_bits_left(data, random.randint(1, 7))  # Approximate inverse
    ...
    def compress_with_best_method(self, data):
        ...
        for marker, transform in transformations:
            try:
                ...
                print(f"Error applying transformation {marker}: {e}")
        if best_compressed is None:
            raise ValueError("No transformation succeeded")
...
def encode_with_compression():
    print("\nSimple Encoder (XOR transformation + PAQ compression)")
    try:
    ...
    try:
        ...
        transformed_data = transform_with_pattern_chunk(original_data)
...
def decode_with_compression():
    print("\nSimple Decoder (PAQ decompression + XOR transformation)")
    try:
    ...
    try:
        ...
        # Apply PAQ decompression
        decompressed_data = paq.decompress(encoded_data)

        # Apply XOR transformation (reverse)
        recovered_data = transform_with_pattern_chunk(decompressed_data)
...
def compress_with_00():
    print("\nIterative Compression (Random Transformations + PAQ)")
    try:
        input_file = input("Enter input file: ").strip()
        output_file = input("Enter output file: ").strip()
        ...
        print("No input detected. Exiting compress mode.")
        return
    ...
    try:
        data = handle_file_io(lambda x: x, input_file)
        if data:
            start_time = time.time()
            compressed_data = compress_with_iterations(data, attempts, iterations)
            end_time = time.time()
            handle_file_io(lambda x: x, output_file, compressed_data)
            print(f"Compressed to {output_file} in {end_time - start_time:.2f} seconds")
    except Exception as e:
        print(f"An error occurred during compression: {e}")

def decompress_with_00():
    print("\nSimple Extraction (PAQ decompression)")
    try:
        input_file = input("Enter input file: ").strip()
        output_file = input("Enter output file: ").strip()
    except EOFError:
        print("No input detected. Exiting extract mode.")
        return
    ...
    try:
        data = handle_file_io(paq.decompress, input_file)
        if data:
            handle_file_io(lambda x: x, output_file, data)
...
def main():
    ...
    print("Options:")
    print("1 - Compress file (PAQJP_1)")
    print("2 - Decompress file (PAQJP_1)")
    print("3 - Encode file (XOR + PAQ)")
    print("4 - Decode file (PAQ + XOR)")
    print("5 - Compress file (Iterative Transformations + PAQ)")
    print("6 - Extract file (PAQ decompression)")
    ...
    try:
        ...
        print("No input detected. Defaulting to Compress (1).")
        choice = '1'
