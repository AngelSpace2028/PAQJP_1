...
def transform_with_pattern(data, repeat=50):
    transformed = bytearray(data)
    for _ in range(repeat):
        for i in range(len(transformed)):
            transformed[i] ^= 0xFF  # Invert all bits
    return bytes(transformed)
...
# Updated SmartCompressor with all four transformations
class SmartCompressor:
    ...
    def transform_02(self, data):
        return transform_with_pattern(data)
    
    def reverse_transform_02(self, data):
        return self.transform_02(data)  # Self-inverse
    
    def transform_03(self, data, repeat=50):
        transformed = bytearray(data)
        for prime in self.PRIMES:
            sub_val = prime if prime == 2 else max(1, math.ceil(prime / 2 / repeat))
            for _ in range(repeat):
                for i in range(0, len(transformed), 3):
                    transformed[i] = (transformed[i] - sub_val) % 256
        return bytes(transformed)
    
    def reverse_transform_03(self, data, repeat=50):
        transformed = bytearray(data)
        for prime in self.PRIMES:
            sub_val = prime if prime == 2 else max(1, math.ceil(prime / 2 / repeat))
            for _ in range(repeat):
