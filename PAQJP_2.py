...
import paq  # Placeholder for PAQ compression library
...
# Prime numbers for transformation (0x03 to 0xFF)
PRIMES = [p for p in range(3, 256) if all(p % d != 0 for d in range(2, int(p**0.5)+1))]
...
def transform_with_prime_xor_every_3_bytes(data, repeat=50):
    ...
    for prime in PRIMES:
        xor_val = max(1, math.ceil(prime / 2 / repeat))
...
def transform_with_table_subtraction(data):
    transformed = bytearray(data)
    table = PRIMES  # Use primes from 0x03 to 0xFF
    for i in range(len(transformed)):
        # Cycle through the table for each byte
        sub_val = table[i % len(table)]
        # Ensure result is not negative
        transformed[i] = max(0, transformed[i] - sub_val)
    return bytes(transformed)

def reverse_transform_with_table_subtraction(data):
    transformed = bytearray(data)
    table = PRIMES  # Use same primes table
    for i in range(len(transformed)):
        add_val = table[i % len(table)]
        # Add back the subtracted value, ensure it stays within byte range
        transformed[i] = min(255, transformed[i] + add_val)
    return bytes(transformed)
...
class SmartCompressor:
    ...
    def compress_with_best_method(self, data):
        ...
        sha256_value = hashlib.sha256(data).digest()  # 32 bytes
        
        # Try all three transformations and select the best (smallest) output
        ...
        transformed_table = transform_with_table_subtraction(data)
        compressed_table = self.paq_compress(transformed_table)
        
        # Select the smallest compressed output
        compressed_sizes = [
            (len(compressed_smart), b'\x01', compressed_smart),  # Smart transform
            (len(compressed_simple), b'\x02', compressed_simple),  # Simple transform
            (len(compressed_table), b'\x03', compressed_table)  # Table subtraction
        ]
        min_size, marker, compressed_data = min(compressed_sizes, key=lambda x: x[0])
        
        # Pack the output: marker (1 byte) + SHA-256 (32 bytes) + compressed data
        return marker + sha256_value + compressed_data
    
    def decompress_with_best_method(self, data):
        if len(data) < 33:  # Need at least 1 byte marker + 32 bytes SHA-256
            raise ValueError("Input data too short to contain marker and SHA-256")
        
        # Extract marker and SHA-256
        method_marker = data[0]
        sha256_stored = data[1:33]
        compressed_data = data[33:]
        
        # Decompress and apply inverse transformation
        decompressed = self.paq_decompress(compressed_data)
        
        if method_marker == 1:
            decompressed = self.reverse_reversible_transform(decompressed)
        elif method_marker == 2:
            decompressed = transform_with_pattern(decompressed)
        elif method_marker == 3:
            decompressed = reverse_transform_with_table_subtraction(decompressed)
        else:
            raise ValueError("Unknown compression method marker")
        
        # Verify SHA-256 to ensure losslessness
        sha256_computed = hashlib.sha256(decompressed).digest()
        if sha256_computed != sha256_stored:
            raise ValueError("SHA-256 mismatch: decompressed data does not match original")
        
        return decompressed
    
    def paq_compress(self, data):
        return paq.compress(data)  # Placeholder for PAQ compression (assumed lossless)
    
    def paq_decompress(self, data):
        return paq.decompress(data)  # Placeholder for PAQ decompression (assumed lossless)

def verify_lossless(original, compressed, compressor):
    """Verify that compression and decompression is lossless."""
    try:
        decompressed = compressor.decompress_with_best_method(compressed)
        return original == decompressed
    except Exception as e:
        print(f"Lossless verification failed: {e}")
        return False
...
def main():
    ...
    if action == "1":
        ...
        # Verify losslessness during compression
        if verify_lossless(input_data, compressed, sc):
            print("Lossless verification during compression: PASS")
        else:
            print("Lossless verification during compression: FAIL")
            return
        ...
        try:
            ...
            print(f"Decompression successful. Output saved to {output_file}. Lossless verification (SHA-256): PASS")
